...
app.get("/tests/:id", async (request, response) => {
  try {
    ...
    const test = await db.query("SELECT * FROM tests WHERE id = $1", [
      id
    ]);
...
app.get("/attempts", async (request, response) => {
  try {
    const allAttempts = await db.query("SELECT * FROM attempts");
...
app.delete("/attempts/:id", async (request, response) => {
  try {
    const { id } = request.params;
    const deleteAttempt = await db.query("DELETE FROM attempts WHERE id = $1", [
      id
    ]);
